...
import { addToUserHistory } from "../../services/api";
...
export default function MovieCard({ movie }: { movie: any }) {
  ...
  return (
    <Card
      onClick={handleClick}
      className="group relative w-56 flex-shrink-0 bg-black border border-gray-800 hover:border-red-500 transition-all duration-500 hover:scale-105 cursor-pointer overflow-hidden rounded-md"
    >
          ...
          <div className="flex flex-wrap gap-1 mb-2">
            {movie.genre?.slice(0, 2).map((g: string) => (
              <Badge
                key={g}
                variant="outline"
                className="text-xs border-gray-600 text-gray-300 bg-black/50"
              >
                {g}
              </Badge>
            ))}
