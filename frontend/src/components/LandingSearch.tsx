...
export default function LandingSearch({ onMovieSelect, onVoiceResults, className = "" }: LandingSearchProps) {
    ...
    <>
          ...
          <ul className="absolute top-full left-0 w-full mt-1 max-h-60 overflow-y-auto bg-black border border-blue-500 rounded-lg shadow-[0_0_10px_rgba(0,0,255,0.7),0_0_20px_rgba(255,0,0,0.7)] z-50">
            {showVoiceResults && (
              <li className="px-4 py-2 border-b border-gray-700">
                <span className="text-blue-400 text-sm font-semibold">🎤 Voice Search Results</span>
              </li>
            )}
            {currentResults.map((movie, index) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-red-400 drop-shadow-[0_0_4px_rgba(255,0,0,0.8)]">{movie.title}</span>
                    {movie.release_date && (
                  ...
                  {showVoiceResults && movie.score && (
                    <span className="text-green-400 text-xs">{(movie.score * 100).toFixed(1)}% match</span>
                  )}
                </div>
              </li>
            ))}
      ...
      <Dialog open={isVoiceModalOpen} onOpenChange={closeVoiceModal}>
        <DialogContent className="bg-gray-900 border-gray-700 text-white">
          <DialogHeader>
            <DialogTitle className="text-center text-xl font-bold">🎤 Voice Search</DialogTitle>
          </DialogHeader>

          <div className="flex flex-col items-center space-y-6 py-6">
            {!isRecording && !isProcessing && (
              <div className="text-center space-y-4">
                <p className="text-gray-300">Describe the type of movie you want to watch</p>
                <p className="text-sm text-gray-400">Example: `&quot;`I want something scary`&quot;` or `&quot;`Show me happy movies`&quot;`</p>
                <Button
            ...
            {isProcessing && (
              <div className="text-center space-y-4">
                ...
                <p className="text-blue-400 font-semibold">Processing your voice...</p>
                <p className="text-sm text-gray-400">Finding perfect movie recommendations</p>
              </div>
            )}
          </div>
        </DialogContent>
